...
|---|-----------|--------------|-----|
| 25 | Throttle cadence N (row 23) | ~~N = 5, fixed~~ **SUPERSEDED by row 28 (randomized).** | Initial lock; the user then asked for a randomized cadence (row 28). |
| 26 | Throttle pause M (row 23) | ~~M = 120000 ms, fixed~~ **SUPERSEDED by row 28 (randomized).** | Initial lock; superseded by row 28. |
| 27 | Detail=NTL when uncaptured | ~~HARD-ERROR~~ **SUPERSEDED by row 29 — the user captured a real `Detail=NTL` URL, so NTL is now wired (token `10`).** The hard-error now applies only to still-uncaptured levels (HS8, …). | Was correct while NTL's token was unknown; the real URL arrived 2026-08-19. |
| 28 | Randomized anti-block throttle (updates rows 23/25/26) | **RANDOM burst + RANDOM pause, both re-drawn every break.** After a random **1–5** countries that ran (`throttleEveryMin`/`throttleEveryMax`), take a random **2–7 min** break (`throttlePauseMinMs`/`throttlePauseMaxMs` = 120000/420000). The pause is taken **BEFORE the next run** (not after), so trailing resume-skips never waste a pause and no pause trails the last country. `throttleEveryMax = 0` disables. RNG injected (`BatchDeps.random`) so it's proven offline. Politeness buffer, NEVER a limit-bypass. | User (2026-08-19): "time gap randomly 2min–7min, after 1–5 countries, and it should keep changing each time" (example 4,1,2,5). Randomized cadence spreads load and is less fingerprintable while staying within the compliance boundary. |
| 29 | Detail=NTL URL token (updates rows 21/22/27) | **NTL = `10`** (in `DETAIL_URL_TOKENS`), captured 2026-08-19 from a real India (`c/699`) imports byProduct URL: `…/byProduct/month/200704-202605/10/direct/values/USD/table`. NTL byProduct exports now build a real URL. HS2/HS4/HS6 = `2`/`4`/`6`. Any still-uncaptured level hard-errors (`DETAIL_TOKEN_UNCAPTURED`) — never invented, never substituted. | User confirmed the `10` segment was the NTL dropdown (2026-08-19). Token read from a real logged-in URL, per CLAUDE.md. |

**Built 2026-08-19** — offline-provable slice: byProduct URL builder + parser (verified byte-for-byte against the real captured URL), Detail token resolver (incl. NTL=10), randomized throttle, NTL now exports (uncaptured levels hard-error), Monthly-PRO warning. Tests: isolation 36, batch 26, runplan 20 (manifest 24 / report 8 unchanged), `tsc` clean. Adversarial multi-agent review: byProduct URL/parser/NTL clean; 3 throttle findings fixed by the randomized redesign.
**Still HEADED (not built):** the live-DOM re-analyse-after-each-answer questionnaire (row 20) and the byProduct `readShownRange`/heading calibration remain the user's headed session; byProduct export (HS6 + NTL) is wired by URL but unverified end-to-end live (does the site accept the explicit range or only `default`?).
